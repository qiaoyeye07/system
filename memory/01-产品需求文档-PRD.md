...
| 项目 | 内容 |
| --- | --- |
| 文档版本 | v1.3（补充退款中状态、交换订单异常流程、自动完成时间、举报功能等细节规则） |
| 产品阶段 | 最小可行产品（MVP）规划 |
...
本产品是一套 C2C 个人对个人的闲置物品交易平台。所有注册用户均可作为买家浏览、购买商品，也可作为卖家发布、管理商品。平台提供商品展示、站内聊天、担保交易（模拟支付）、以物易物、举报和基础评价功能。产品首期服务于小规模本地演示和试用，不以生产级规模和完整电商运营能力为目标。
...
| 编号 | 功能需求 | 适用角色 | 优先级 |
| --- | --- | --- | --- |
| FR-RPT-01 | 用户应能举报违规商品（虚假描述、违禁品等），入口位于商品详情页。 | 普通用户 | 必须 |
| FR-RPT-02 | 用户应能举报违规用户（欺诈、骚扰等），入口位于用户主页。 | 普通用户 | 必须 |
| FR-RPT-03 | 用户应能举报违规聊天消息（骚扰、辱骂等），入口位于聊天页每条消息旁。 | 普通用户 | 必须 |
| FR-RPT-04 | 举报时应提供举报原因（预设选项）和补充描述（可选）。 | 普通用户 | 必须 |
| FR-RPT-05 | 管理员应能查看举报列表并按状态（待处理/处理中/已受理/已驳回/申诉中）筛选处理。 | 管理员 | 必须 |
| FR-RPT-06 | 管理员受理举报后可执行：下架商品、禁用用户、驳回举报。 | 管理员 | 必须 |
| FR-RPT-07 | 被举报人收到处理后可提出申诉；管理员可维持原判或改判。 | 普通用户、管理员 | 必须 |
| FR-RPT-08 | 用户应能查看自己提交的举报记录及处理结果。 | 普通用户 | 必须 |
...
| --- | --- |
| 在售 | 商品正常展示，可被浏览和购买。 |
| 已售 | 商品已被购买，不再公开展示。若关联订单取消或退款成功，商品自动恢复为"在售"。 |
| 已下架 | 卖家主动下架，不再公开展示。 |
...
| 状态 | 产品含义 |
| --- | --- |
| 待确认 | 买家发起交换提议，等待卖家确认；买家可主动撤回。 |
| 已确认 | 卖家同意交换，等待双方发货；任意一方可申请协商取消。 |
| 双方已发货 | 双方均已发货，等待双方收货。 |
| 已完成 | 双方均确认收货，交换完成。 |
| 已拒绝 | 卖家拒绝交换提议，为终态；买家可重新发起新提议。 |
| 已取消 | 买家撤回提议或双方协商取消，为终态；双方商品恢复在售。 |

### 7.6 举报流程

```mermaid
flowchart TD
    A["用户在商品详情/聊天/用户主页点击举报"] --> B["填写举报原因和补充描述"]
    B --> C["提交举报，状态：待处理"]
    C --> D["管理员查看举报列表"]
    D --> E{"管理员裁定"}
    E -- "受理：下架商品/禁用用户" --> F["状态变为：已受理；通知举报人与被举报人"]
    E -- "驳回" --> G["状态变为：已驳回；通知举报人"]
    F --> H{"被举报人是否申诉？"}
    H -- "申诉" --> I["状态变为：申诉中"]
    I --> J{"管理员改判？"}
    J -- "维持原判" --> K["状态变为：已受理（终审）"]
    J -- "改判" --> L["状态变为：已驳回（改判）"]
    H -- "不申诉" --> M["状态保持：已受理"]
```
...
| 待处理 | 举报已提交，等待管理员处理。 |
| 处理中 | 管理员正在查看处理中。 |
| 已受理 | 管理员确认举报成立，已执行相应处理（下架商品/禁用用户）。 |
| 已驳回 | 举报不成立，不予处理。 |
| 申诉中 | 被举报人不服处理结果，提出申诉。 |
...
| 举报处理页 | 处理用户举报 | 查看举报列表、查看详情、做出处理/驳回 |
...
- 买家确认收货后，若 3 天内未发起退款申请，订单自动变更为"已完成"。
- 订单取消或退款成功后，商品自动恢复为"在售"状态。
...
| AC-16 | 用户可举报违规商品、用户或聊天消息，填写举报原因和描述。 |
| AC-17 | 管理员可查看举报列表、处理举报（下架商品/禁用用户/驳回），处理结果通知相关人员。 |
| AC-18 | 被举报人可对处理结果提出申诉，管理员可维持原判或改判。 |
| AC-19 | 用户可查看自己提交的举报记录及处理结果。 |
